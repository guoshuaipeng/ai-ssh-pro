...
export default function AiConfigSection() {
  ...
  return (
    <div style={{ marginBottom: 16, borderBottom: '1px solid var(--border)', paddingBottom: 12 }}>
      ...
      {open && (
        <>
          <p style={{ margin: '8px 0', fontSize: 11, color: 'var(--muted)', lineHeight: 1.4 }}>
            API Key 仅存于本机主进程（electron-store）。对话与「智能填表」共用下列接口。
          </p>
          <div className="field">
          ...
          {savedHint && (
            <span style={{ marginLeft: 8, fontSize: 12, color: 'var(--accent)' }}>{savedHint}</span>
          )}
        </>
      )}
    </div>
